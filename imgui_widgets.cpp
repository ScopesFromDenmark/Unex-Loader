...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    //{
    //    col_bg = ImGui::GetColorU32(ImLerp(ImColor(1, 1, 1), ImVec4(0.77f, 0.65f, 0.98f, 1.0f), t));
    //    checkbg = ImGui::GetColorU32(ImLerp(ImVec4(1.00f, 1.00f, 1.00f, 1.0f), ImVec4(0.39f, 0.08f, 0.96f, 1.0f), t));
    //}

    //else
   // {
    col_bg = ImGui::GetColorU32(ImLerp(ImColor(81, 84, 90), ImVec4(1.00f, 0.19f, 0.19f, 0.90f), t));
    checkbg = ImGui::GetColorU32(ImLerp(ImColor(173, 182, 189), ImVec4(1.00f, 0.19f, 0.19f, 0.90f), t));
    // }

    draw_list->AddRectFilled(ImVec2(p.x, p.y + 6), ImVec2(p.x + width - 8, p.y + height - 6), col_bg, 100);
    draw_list->AddCircleFilled(ImVec2(p.x + radius + t * (width - radius * 1.8f) - 6, p.y + radius), radius - 1.5f, checkbg, 100);


    if (label_size.x > 0.0f)
    {
        RenderText(ImVec2((p.x + width) + 7, (p.y + height) - 20), label);
    }
}
